...
import intellispaces.templateengine.expression.compilation.CompileFunctions;
import intellispaces.templateengine.expression.value.BooleanValueBuilder;
import intellispaces.templateengine.expression.value.IntegerValueBuilder;
import intellispaces.templateengine.expression.value.ListValueBuilder;
import intellispaces.templateengine.expression.value.MapValueBuilder;
import intellispaces.templateengine.expression.value.RealValueBuilder;
import intellispaces.templateengine.expression.value.StringValueBuilder;
import intellispaces.templateengine.expression.value.Value;
import intellispaces.templateengine.expression.value.VoidValues;
...
public final class ParseExpressionFunctions {
  ...
  private static String prepareStatement(
    ...
    while (ind < chars.length) {
      ...
      } else if (TextFunctions.isWordChar(curChar)) {
  ...
  private static String readWord(char[] chars, int beginIndex) {
    int ind = beginIndex;
    while (ind < chars.length && TextFunctions.isWordChar(chars[ind])) {
      ind++;
    }
    return new String(chars, beginIndex, ind - beginIndex);
  }
