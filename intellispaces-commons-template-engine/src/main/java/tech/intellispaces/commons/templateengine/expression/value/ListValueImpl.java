...
import tech.intellispaces.commons.templateengine.exception.IrregularValueTypeExceptions;
...
class ListValueImpl extends AbstractValue implements ListValue {
  ...
  public Value get(Value key) throws ResolveTemplateException {
    if (key.type() != ValueTypes.Integer) {
      throw IrregularValueTypeExceptions.withMessage("Invalid index type: {0}. Expected integer value", key.typename().get());
    }
